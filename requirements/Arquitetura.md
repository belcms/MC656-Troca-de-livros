...
A arquitetura do projeto foi desenhada visando a separação de responsabilidades, alta coesão e baixo acoplamento, garantindo um sistema escalável e de fácil manutenção.
...
- **Módulos de Interface por Funcionalidade**: o frontend é organizado por funcionalidades, como `feed`, `book_creation`, `book_edition`, `book_details`, `search`, `favorites`, `trade_requests`, `user_profile` e `auth`. Cada módulo concentra as telas, widgets e modelos relacionados à sua responsabilidade principal.
...
- **Camada de Domínio do Backend**: concentra as regras de negócio da aplicação. Inclui funcionalidades como gerenciamento de usuários, autenticação, livros, edições, anúncios, busca, localização, favoritos e propostas de troca.
